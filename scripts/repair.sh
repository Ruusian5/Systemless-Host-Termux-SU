...
cleanup_repair() {
  echo -e "\n${C_RED}[!] Repair interrupted.${NC}" >&2
  exit 1
}
trap cleanup_repair INT TERM

if [ ! -d "$DEBIANPATH" ]; then
  echo -e "${C_RED}[Repair Tool v0.1] Missing chroot: $DEBIANPATH${NC}" >&2
  exit 1
fi
...
su -c "for i in \/sys\/devices\/system\/cpu\/cpu*\/cpufreq\/scaling_governor; do [ -w \"\$i\" ] \&\& echo performance > \"\$i\"; done" 2>/dev/null && echo -e " [✓] All Cores set to PERFORMANCE." || true
