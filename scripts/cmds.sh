#!/bin/bash
# --- PRO WORKSTATION DASHBOARD v3.2 ---
# Updated: July 8 2026 — Turnip+Zink GPU, duplicate GUI prevention, Synaptic as primary software manager
...
echo "╔══════════════════════════════════════════════╗"
echo "║     PRO WORKSTATION DASHBOARD v3.2          ║"
    ...
    case $opt in
        ...
        9) echo -e "${C_YELLOW}[~] Launching Synaptic Package Manager...${NC}"
           if su -c "test -x /data/local/tmp/chrootDebian/usr/sbin/synaptic" 2>/dev/null; then
               nohup su -c "chroot /data/local/tmp/chrootDebian /usr/bin/env -i DISPLAY=:0 HOME=/home/ruusian TERM=xterm PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin /bin/su -l ruusian -c 'synaptic'" > /dev/null 2>&1 &
               disown
               echo -e "${C_GREEN}[✓] Synaptic launched on the desktop${NC}"
           else
               echo -e "${C_RED}[✗] Synaptic not installed in chroot${NC}"
           fi ;;
        10) echo -e "${C_YELLOW}[~] Restarting GUI...${NC}"
