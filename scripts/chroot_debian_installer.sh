...
main() {
    DEBIANPATH="/data/local/tmp/chrootDebian"
    if [ -f "$DEBIANPATH/etc/debian_version" ]; then
        echo -e "\e[1;33m[!] Debian system already detected at $DEBIANPATH\e[0m"
        echo -e "\e[1;33m[!] Exiting to prevent accidental data loss.\e[0m"
        exit 0
    fi
