...
if [ ! -d "$DEBIANPATH" ]; then
  echo "[startxfce4] Missing chroot: $DEBIANPATH" >&2
  exit 1
fi
command -v termux-x11 >/dev/null 2>&1 || { echo "[startxfce4] termux-x11 not found" >&2; exit 1; }
...
rm -rf "$TERMUX_TMP"/.X0-lock "$TERMUX_TMP"/.X11-unix/X0 2>/dev/null || true
...
XDG_RUNTIME_DIR=${TERMUX_TMP} termux-x11 :0 -ac -legacy-drawing -disable-dri3 -legacy-drawing -disable-dri3 > "$HOME/x11_server.log" 2>&1 &

echo -e "\e[1;36m[→] Waiting for Graphics Socket...\e[0m"
sleep 2
chmod 666 "$TERMUX_TMP"/.virgl_test 2>/dev/null || true
chmod 666 "$TERMUX_TMP"/.X11-unix/X0 2>/dev/null || true
...
chmod 666 "$TERMUX_TMP/.X11-unix/X0" || true
echo -e "\e[1;32m[✓] Graphics Bridge Established.\e[0m"

if [ -x "$REPO/scripts/clipboard-sync.sh" ]; then
  echo -e "\e[1;32m[+] Launching Universal Clipboard Sync...\e[0m"
  bash "$REPO/scripts/clipboard-sync.sh" > "$HOME/clipboard.log" 2>&1 &
else
  echo -e "\e[1;33m[!] clipboard-sync.sh not found, skipping.\e[0m"
fi

echo -e "\e[1;32m[+] Mounting Debian chroot...\e[0m"
bash "$REPO/scripts/mount-debian.sh"
